...
real_FFT_out_matlab=real(FFT_out_matlab);
imag_FFT_out_matlab=imag(FFT_out_matlab);
real_int16_FFT_out_matlab=aie_srs(real_FFT_out_matlab, 0, 16);
imag_int16_FFT_out_matlab=aie_srs(imag_FFT_out_matlab, 0, 16);
...
Y=abs(X)/max(abs(int16_FFT_out_matlab));
figure(2)
...
title("error rate")
hold on
plot(1:length(FFT_out_hardware),Y);